...
for k in range(ITERATIONS): 
    ...
    number_of_coherentsignals[k] = data_gen.generate_random_number(MIN_NUMBER_OF_SIGNALS, MAX_NUMBER_OF_SIGNALS )
    
    if number_of_coherentsignals[k] == 1:
        number_of_correlatedsources[k] = 0
    else :
        number_of_correlatedsources[k] = data_gen.generate_random_number(MIN_NUMBER_OF_SIGNALS,number_of_coherentsignals[k]  )
    number_of_sources[k] = number_of_correlatedsources[k] + number_of_coherentsignals[k]
    signal_dir = data_gen.generate_random_directions(-SPECTRUM_WIDTH, SPECTRUM_WIDTH, number_of_coherentsignals[k])
    ...
    X_data[k,:] = norm_x
    Y_data[k,:] = number_of_coherentsignals[k] # here it should be number of sources, keep it as a variable inside loop. 
...
import h5py
hf = h5py.File('0-9source_10snr_10k_correlated.h5','w')#change this 
